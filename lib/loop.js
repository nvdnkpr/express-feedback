...
_ex.medianUse = function(action_type, cb) {
  _ex.client.zcard('express-feedback-loop:action:' + action_type, function(err, nbUsers) {

    var mid = Math.floor(nbUsers/2);
    var args = ['express-feedback-loop:action:' + action_type, mid , mid, 'WITHSCORES'];

    _ex.client.zrevrange(args, function(err, median) {
      cb(err, median[1]);
    });
  });
...
_ex.fullStats = function(cb) {
  ...
  _ex.list(function(err, obj) {
    ...
    async.parallel([
      ...
      function(cb) {
        async.map(obj, _ex.medianUse, cb);
      }
    ], function(err, stats) {
